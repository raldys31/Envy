...
public class Charizard extends BaseHostileEntity implements Fighter{
    ...
    public Charizard(Handler handler, int xPosition, int yPosition, String state, String name, String area, BufferedImage[] animFrames) {
        ...
        animDown = new Animation(animWalkingSpeed, Images.charizard_front);
		animLeft = new Animation(animWalkingSpeed, Images.charizard_left);
		animRight = new Animation(animWalkingSpeed, Images.charizard_right);
		animUp = new Animation(animWalkingSpeed, Images.charizard_back);
        
        this.foundState = state;
        charizard = new Rectangle();
    }
    ...
    public void render(Graphics g) {
        ...
        if(handler.getArea().equals(this.Area)) {
            if (!Player.checkInWorld) {
                charizard = new Rectangle((int) (handler.getXDisplacement() + getXOffset()),
                        (int) (handler.getYDisplacement() + getYOffset()), 45, 45);

            } else {
                charizard = new Rectangle((int) (handler.getXInWorldDisplacement() + getXOffset()),
            ...
            g.drawImage(getCurrentAnimationFrame(animDown,animUp,animLeft,animRight,Images.charizard_front,Images.charizard_back,Images.charizard_left,Images.charizard_right), charizard.x,charizard.y,charizard.width,charizard.height,null);

            if (charizard.intersects(handler.getEntityManager().getPlayer().getCollision())) {
