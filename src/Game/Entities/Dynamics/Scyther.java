...
public class Scyther extends BaseHostileEntity implements Fighter{
    ...
    public Scyther(Handler handler, int xPosition, int yPosition, String state, String name, String area, BufferedImage[] animFrames) {
        ...
        animDown = new Animation(animWalkingSpeed, Images.scyther_front);
		animLeft = new Animation(animWalkingSpeed, Images.scyther_left);
		animRight = new Animation(animWalkingSpeed, Images.scyther_right);
		animUp = new Animation(animWalkingSpeed, Images.scyther_back);
        
        this.foundState = state;
        scyther = new Rectangle();
    }
    ...
    public void render(Graphics g) {
        ...
        if(handler.getArea().equals(this.Area)) {
            if (!Player.checkInWorld) {
                scyther = new Rectangle((int) (handler.getXDisplacement() + getXOffset()),
                        (int) (handler.getYDisplacement() + getYOffset()), 45, 45);

            } else {
                scyther = new Rectangle((int) (handler.getXInWorldDisplacement() + getXOffset()),
            ...
            g.drawImage(getCurrentAnimationFrame(animDown,animUp,animLeft,animRight,Images.scyther_front,Images.scyther_back,Images.scyther_left,Images.scyther_right), scyther.x,scyther.y,scyther.width,scyther.height,null);

            if (scyther.intersects(handler.getEntityManager().getPlayer().getCollision())) {
